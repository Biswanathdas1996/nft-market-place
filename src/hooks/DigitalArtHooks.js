...
export const useFindArtTokens = (props) => {
  ...
  useEffect(() => {
    const findArt = async (_web3, tokenId) => {
      const artFind = await contractInstance.methods.findArt(tokenId).call();
      let [
        id,
        title,
        desc,
    ...
    const loadDigitalArts = async (findArts) => {
      try {
        ...
        const tokenIds = Array.isArray(result)
          ? result
          : Object.values(result)[0];
        const _artTokens = await Promise.all(
          tokenIds.map((tokenId) => findArt(web3, tokenId))
        );
    ...
    if (!!web3) {
      const criteria =
        filter === "mine"
          ? contractInstance.methods.findMyArts
          : contractInstance.methods.findAllPendingArt;
...
export const useCreateTokenAndSellArt = (props) => {
  ...
  const createTokenAndSellArt = useCallback(
    async (artToken) => {
      const { title, description, date, authorName, price, imageValue } =
        artToken;
      const { utils } = web3;
